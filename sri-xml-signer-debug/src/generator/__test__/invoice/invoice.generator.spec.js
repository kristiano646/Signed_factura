...
jest.mock("../../../lightweight-validator/validations/object.validation", () => ({
...
describe("generateXmlInvoice", () => {
    ...
    it("debe generar un XML correctamente si no hay errores", async () => {
        const result = await generateXmlInvoice(baseInvoice);
        ...
        expect(result.invoiceJson.factura.infoTributaria.razonSocial).toBe("Empresa de Prueba");
    });
    it("debe lanzar ValidationException si hay errores de validación", async () => {
        const { validateObject, } = require("../../../lightweight-validator/validations/object.validation");
        validateObject.mockReturnValueOnce(["campo requerido"]);
        await expect(generateXmlInvoice(baseInvoice)).rejects.toThrow(ValidationException);
    });
});
