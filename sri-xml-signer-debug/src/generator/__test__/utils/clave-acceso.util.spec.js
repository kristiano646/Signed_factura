...
import { p_calcular_digito_modulo11, p_obtener_codigo_autorizacion, pad, } from "../../invoice/utils";
// Mock de los formatos (basado en la imagen)
jest.mock("../../../utils", () => ({
    formatDDMM: (date) => "0107",
    formatDDMMYYYY: (date) => "01072025",
}));
...
describe("p_calcular_digito_modulo11", () => {
    it("devuelve el dígito correcto para una cadena válida", () => {
        const digito = p_calcular_digito_modulo11("01072025011234567890001100000012301071");
...
describe("p_obtener_codigo_autorizacion", () => {
    it("genera un código de autorización válido con valores por defecto", () => {
        const result = p_obtener_codigo_autorizacion(new Date("2025-07-01"), undefined, // tipoComprobante
        undefined, // ruc
        undefined, // ambiente
        undefined, // estab
        undefined, // ptoEmi
    ...
    it("utiliza el tipo de comprobante 'factura' por defecto", () => {
        const result = p_obtener_codigo_autorizacion(new Date("2025-07-01"), undefined, undefined, undefined, undefined, undefined, "123");
        expect(result.startsWith("01072025")).toBe(true);
    });
    it("devuelve el mismo código si se proporciona 'codigo'", () => {
        const customCode = "12345678";
        const result = p_obtener_codigo_autorizacion(new Date("2025-07-01"), "factura", "1234567890001", ENV_ENUM.TEST, 1, 1, "123", customCode);
    ...
    it("lanza error si no se pasa el secuencial", () => {
        expect(() => p_obtener_codigo_autorizacion(new Date("2025-07-01"), "factura", "1234567890001", ENV_ENUM.TEST, 1, 1, "" // secuencial faltante
