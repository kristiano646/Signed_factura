...
describe("ExclusiveCanonicalisation", () => {
    ...
    it("should return the correct canonicalization algorithm name", () => {
        expect(canonicalizer.name()).toBe("http://www.w3.org/2001/10/xml-exc-c14n#WithComments");
    });
    ...
    it("should canonicalize a basic XML element with attributes", async () => {
        ...
        const result = await canonicalizer.canonicalise(doc.documentElement);
    ...
    it("should process processing instruction nodes", async () => {
        ...
        const doc = new DOMParser().parseFromString(xml, "text/xml");
        const result = await canonicalizer.canonicalise(doc);
        expect(result).toContain("");
    });
    ...
    it("should render empty result for unsupported node type", async () => {
        ...
        const result = await canonicalizer._processInner(cdata);
        expect(result).toBe("");
    });
});
describe("_compareAttributes", () => {
    const canonicalizer = new ExclusiveCanonicalisation();
    const createAttr = (name, prefix = null) => {
        return {
            name,
            prefix,
            value: "",
    ...
    it("debería retornar -1 si a tiene prefix y b no", () => {
        ...
        const result = canonicalizer._compareAttributes(a, b);
...
describe("_compareNamespaces", () => {
    ...
    it("debería ordenar por prefix + namespaceURI", () => {
        ...
        const result = canonicalizer._compareNamespaces(ns1, ns2);
...
describe("ExclusiveCanonicalisation", () => {
    const createElementFromString = (xmlStr) => {
    ...
    it("should canonicalize element with new prefix in scope", async () => {
        ...
        const element = createElementFromString(`<foo:bar xmlns:foo="http://test.com"/>`);
        const result = await canonicalizer.canonicalise(element);
        expect(result).toContain('xmlns:foo="http://test.com"');
        expect(result).toContain("<foo:bar");
    });
    it("should canonicalize with default namespace changed", async () => {
        const canonicalizer = new ExclusiveCanonicalisation();
        const element = createElementFromString(`<bar xmlns="http://default.com"/>`);
        const result = await canonicalizer.canonicalise(element);
        expect(result).toContain('xmlns="http://default.com"');
    });
    it("should render attribute with prefix not in scope", async () => {
        const canonicalizer = new ExclusiveCanonicalisation();
        const element = createElementFromString(`<test xmlns:abc="http://abc.com" abc:attr="123"/>`);
        const result = await canonicalizer.canonicalise(element);
...
it("should render processing instruction node", async () => {
    ...
    const piNode = Array.from(doc.childNodes[0].childNodes).find((n) => n.nodeType === 7 // PROCESSING_INSTRUCTION_NODE
    );
    const result = await canonicalizer["canonicalise"](piNode);
