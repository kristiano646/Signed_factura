...
export class XadesSignatureService {
  ...
  async sign(data: SignatureData): Promise<XadesBesResultInterface> {
    ...
    console.log("✅ ¿Coinciden?:", sha256_SignedProperties === certDigest ? "SÍ - FORZADO" : "NO");
    ...
    // 🔍 DEBUG: Verificar el SignedInfo original
    console.log("📄 SignedInfo ORIGINAL (primeros 300 chars):", SignedInfo.substring(0, 300));
    console.log("📄 SignedInfo ORIGINAL COMPLETO:", SignedInfo);
    
    const signatureValue = this.signer.signSha256RsaBase64(canonicalizedSignedInfo);
    
    // 🔍 DEBUG: Verificar la firma generada
    console.log("🔐 SignatureValue generado:", signatureValue);
    console.log("🔐 Longitud SignatureValue:", signatureValue.length);
