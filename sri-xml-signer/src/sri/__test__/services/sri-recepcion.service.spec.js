...
describe("validarComprobante", () => {
    ...
    beforeEach(() => {
        jest.clearAllMocks();
        helpers.createSoapClient.mockResolvedValue(mockClient);
    });
    ...
    it("debería lanzar SRIRejectedError si el comprobante no fue recibido con mensaje", async () => {
        ...
        mockClient.validarComprobanteAsync.mockResolvedValue([mockRespuesta]);
        await expect(validateXml({ xml: mockXml, env: "test" })).rejects.toThrow(SRIRejectedError);
    });
    it("debería lanzar Error si comprobante no fue recibido y no hay mensaje", async () => {
        ...
        mockClient.validarComprobanteAsync.mockResolvedValue([mockRespuesta]);
        await expect(validateXml({ xml: mockXml, env: "test" })).rejects.toThrow("Comprobante no recibido y sin mensaje explicativo del SRI");
    });
    it("debería lanzar Error si respuesta SOAP no tiene RespuestaRecepcionComprobante", async () => {
        ...
        mockClient.validarComprobanteAsync.mockResolvedValue([mockRespuesta]);
        await expect(validateXml({ xml: mockXml, env: "test" })).rejects.toThrow("Respuesta inválida del SRI");
    });
    it("debería lanzar Error si hay un fallo en el cliente SOAP", async () => {
        mockClient.validarComprobanteAsync.mockRejectedValue(new Error("Conexión fallida"));
        await expect(validateXml({ xml: mockXml, env: "test" })).rejects.toThrow("Error SOAP al validar comprobante: Conexión fallida");
    });
});
