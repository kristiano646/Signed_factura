...
import { PrivateKeyExtractionError, SigningKeyNotFoundError, EntidadCertificacionNotFoundError, } from "../../errors";
export class EntidadCertificacionStrategy {
    ...
    async getPrivateKey(bags) {
        ...
        const item = bags[0];
        if (!item)
            throw new SigningKeyNotFoundError("ENTIDAD DE CERTIFICACION DE INFORMACION");
    ...
    async getX509IssuerName(cert) {
        ...
        const findAttribute = (shortName, type) => {
            return issuerAttributes.find(attr => (attr.shortName === shortName) ||
                (type && attr.type === type));
        };
        // 1. L (Locality)
        const lAttr = findAttribute('L');
        if (lAttr)
            orderedAttributes.push(`L=${lAttr.value}`);
        // 2. CN (Common Name)
        const cnAttr = findAttribute('CN');
        if (cnAttr)
            orderedAttributes.push(`CN=${cnAttr.value}`);
        // 3. ST (State/Province)
        const stAttr = findAttribute('ST');
        if (stAttr)
            orderedAttributes.push(`ST=${stAttr.value}`);
        // 4. OU (Organizational Unit)
        const ouAttr = findAttribute('OU');
        if (ouAttr)
            orderedAttributes.push(`OU=${ouAttr.value}`);
        // 5. O (Organization)
        const oAttr = findAttribute('O');
        if (oAttr)
            orderedAttributes.push(`O=${oAttr.value}`);
        // 6. C (Country)
        const cAttr = findAttribute('C');
        if (cAttr)
            orderedAttributes.push(`C=${cAttr.value}`);
        // 7. E (Email) - si existe
        const eAttr = findAttribute('E') || findAttribute(undefined, 'emailAddress');
        if (eAttr)
            orderedAttributes.push(`E=${eAttr.value}`);
