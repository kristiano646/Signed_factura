...
describe("SignStrategyFactory", () => {
    ...
    it("debería retornar la estrategia que soporta el friendlyName", () => {
        ...
        const uanataca = factory["strategies"].find((s) => s instanceof UanatacaStrategy);
    ...
    it("debería lanzar UnknownSignStrategyError si ninguna estrategia lo soporta", () => {
        ...
        factory["strategies"].forEach((strategy) => jest.spyOn(strategy, "supports").mockReturnValue(false));
        expect(() => factory.getStrategy("desconocido")).toThrow(UnknownSignStrategyError);
        expect(() => factory.getStrategy("desconocido")).toThrow("No existe estrategia para el certificado: desconocido");
    });
});
