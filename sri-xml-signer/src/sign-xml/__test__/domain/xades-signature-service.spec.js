...
const signatureData = {
    ...
    certData: {
        ...
        certificate: {}, // opcional si no se usa directamente
        privateKey: {}, // opcional si no se usa directamente
    },
};
describe("XadesSignatureService", () => {
    let service;
    beforeEach(() => {
        service = new XadesSignatureService(clock, canonicalizer, hasher, idGenerator, signer);
    });
    it("debería generar una firma XAdES-BES correctamente", async () => {
        const result = await service.sign(signatureData);
        expect(idGenerator.generateAll).toHaveBeenCalled();
        expect(canonicalizer.canonicalize).toHaveBeenCalledWith("<Factura>contenido</Factura>");
        expect(hasher.sha1Base64).toHaveBeenCalledWith(expect.stringContaining("<Canon>"));
        expect(hasher.sha1Base64).toHaveBeenCalledWith(expect.stringContaining("SignedProperties"));
        expect(hasher.sha1Base64).toHaveBeenCalledWith(expect.stringContaining("KeyInfo"));
