...
export async function signXml(cmd) {
    try {
        await assertIsValidP12OrThrow(cmd.p12Buffer, cmd.password);
        const certProvider = new CertificateProviderImplement(cmd.p12Buffer, cmd.password, new SignStrategyFactory(), new CryptoUtils());
